...
public class BoardplayEventPainter extends EventPainter {
	private Dimension dimension;
	
	public BoardplayEventPainter(Event event, String level) {
		super(event, level);
	}
	
	public BoardplayEventPainter(Event event, String description, List<Match> matches) {
	...
	public void paint(int matchHeight, int xPadding, int yPadding, float fontSize, int textPadding, Graphics g) {
		...
		// int size = (matches.size() * 2 * matchHeight) / (currentMatches.size() * 2);
                // 12/10//19 bvw fudge fix size, currentMatches
                int size=matchHeight;
                currentMatches = new ArrayList<Match>(matches);
