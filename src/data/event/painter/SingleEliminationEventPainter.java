...
public class SingleEliminationEventPainter extends EventPainter {
	...
	public void paint(int matchHeight, int xPadding, int yPadding, float fontSize, int textPadding, Graphics g) {
		...
		int levels = 0;
		int matchWidth = 0;
		List<Match> matches = getEvent().getMatches(getLevel());
		...
		while(!currentMatches.isEmpty()) {
			...
			if(pre != null && match.getLevel().equals(pre.getLevel())) {
				nextMatches.add(pre);
			}
			if(!match.getIgnoreMatch()) {
				...
				else if(pre == null || !pre.equals(match.getToTeam1Match(false))) {
			...
			if(!match.getIgnoreMatch()) {
				...
				else if(pre == null || !pre.equals(match.getToTeam2Match(false))) {
