  	...
  	private static final long serialVersionUID = 2564561655115801740L;
		...
		for(String teamTypeCode : list) {
			...
			setTeamType((String) type.getSelectedItem());
                ...
                int Fcount=0; int Mcount=0; // index
                int sumFGrade=0;int sumMGrade=0;  // total of all player grades
		for(Player player : tournamentViewManager.getTournament().getPlayers()) {
			if(player != null && player.isCheckedIn()) {
                                ...
                                players.add(playerCount,playerData);
                                if(player.isMale())  { 
                                    Mcount++;
                                    sumMGrade+=calcLevel; 
                                    } 
                ...
                playerCount=Mcount+Fcount;
                gradeAvg=sumFGrade/Fcount+sumMGrade/Mcount;
        ...
        int gradeDiff1 = abs((this.gradeAvg)-(Integer.valueOf(player1.getLevel())+Integer.valueOf(pairCandidate.getLevel())));  
        int gradeDiff2 = abs((this.gradeAvg)-(Integer.valueOf(player1.getLevel())+Integer.valueOf(player2.getLevel())));   
